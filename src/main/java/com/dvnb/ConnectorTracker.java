...
//	private static final boolean fineLogging = getLogger().isLoggable(Level.FINE);
//
...
//            getLogger().log(Level.WARNING, "An already registered connector was registered again: {0} ({1})",
//                    new Object[] { connector.getClass().getSimpleName(), connectorId });
//        }
...
//            getLogger().log(Level.WARNING, "Tried to unregister {0} ({1}) which is not registered",
//                    new Object[] { connector.getClass().getSimpleName(), connectorId });
//            return;
//        }
//        if (connectorIdToConnector.get(connectorId) != connector) {
//            throw new RuntimeException("The given connector with id " + connectorId
...
//            removeUnregisteredConnector(connector, uI.getSession().getGlobalResourceHandler(false));
...
//            }
//        } else {
//            getLogger().log(Level.WARNING, "Unregistered {0} ({1}) that was already unregistered.",
//                    new Object[] { connector.getClass().getSimpleName(), connectorId });
//        }
//    }
//
...
//                uninitializedConnectors.add(connector);
//                diffStates.remove(connector);
//                assert isRemovalSentToClient(connector) : "Connector " + connector + " (id = "
//                        + connector.getConnectorId()
//                        + ") is no longer visible to the client, but no corresponding hierarchy change was sent.";
//                if (fineLogging) {
//                    getLogger().log(Level.FINE, "cleanConnectorMap removed state for {0} as it is not visible",
//                            getConnectorAndParentInfo(connector));
//                }
//            }
//        }
...
//        Iterable<? extends ClientConnector> iterable = AbstractClientConnector.getAllChildrenIterable(parent);
